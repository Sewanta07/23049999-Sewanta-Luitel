    ...
    public partial class Tickets_TicketForm : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    SetMessage("Unable to load ticket details.", true);
                }
            }
        }
        ...
        protected void btnBook_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(ddlUsers.SelectedValue) ||
                string.IsNullOrWhiteSpace(ddlShows.SelectedValue) ||
                string.IsNullOrWhiteSpace(txtSeatNumber.Text) ||
                string.IsNullOrWhiteSpace(txtTicketPrice.Text))
            {
                SetMessage("User, show, seat number, and ticket price are required.", true);
                return;
            }
            ...
            {
                SetMessage("Ticket price must be a positive value.", true);
                return;
            }
                    ...
                    try
                    {
                        if (TicketId.HasValue)
                        {
                            ...
                            using (OracleCommand cmd = new OracleCommand("SELECT Booking_Id FROM TICKET WHERE Ticket_Id = :Ticket_Id", con))
                            {
                                ...
                                if (result == null)
                                {
                                    SetMessage("Ticket not found for update.", true);
                                    return;
                                }
            ...
            {
                SetMessage("Unable to save ticket.", true);
            }
        }
